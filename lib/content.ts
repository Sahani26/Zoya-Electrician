...
const areas = ["Adajan", "Vesu", "Piplod", "Pal", "Jahangirpura", "Gaurav Path Road", "Citylight", "Althan", "Athwa", "Bhatar", "Udhna", "Dindoli"];
...
export const areaPages: Page[] = areas.map((area, index) => page("area", `/areas/electrician-in-${area.toLowerCase().replace(/\s+/g, "-")}`, `Electrician in ${area}, Surat`, `Professional electrician services in ${area}, Surat for repairs, wiring, lighting, safety and planned electrical work.`, "YOUR LOCAL ELECTRICIAN", `Practical, polished electrical service for homes and businesses in ${area}.`, [
